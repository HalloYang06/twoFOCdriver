...
本目录集中放置 h7FOC 的调试记录、联调手册、协议移植说明和面试展示资料。根目录 README 负责快速讲清项目，本目录负责让面试官或协作者继续深入追问。
...
4. [电机不转排查指南](DEBUG_INSTRUCTIONS.md)：展示调试方法，而不是只展示最后结果。
5. [HardFault 复盘](HardFault_问题复盘.md)：展示 Cortex-M7、HAL 状态机、DMA/Cache 和内存安全意识。
6. [Modbus 联调手册](Modbus_TEST1一致性核对与联调手册.md) 和 [EtherCAT 移植手册](EtherCAT_开发移植学习手册.md)：展示工业通信协议联调经验。
...
| 文档 | 内容 |
| --- | --- |
| [interview_project_showcase.html](interview_project_showcase.html) | 可直接打开的项目展示页，适合面试或作品集说明 |

## 文档维护约定

- 调试、复盘、联调和展示资料统一放在 `docs/`。
- 根目录只保留 `README.md`、构建配置、工程配置和源码目录。
- 新增调试文档时同步更新本索引和根目录 README 的“面试官快速跳转”区域。
